import { Settings, TASK_NAME } from '@/entities'
...
class ImportTaskClass extends BaseTask {
  ...
  async run() {
    ...
    const omdbMetadata = new TMDbMetadataAgent(settings.tmdbKey)
    const tmdbMetadata = new OMDbMetadataAgent(settings.omdbKey)
