...
export abstract class PlexSection<T> {
  ...
  constructor(public library: PlexLibrary, public key: string, public uuid: string) {
...
export class MovieSection extends PlexSection<PlexMovie> {
  ...
  constructor(public library: PlexLibrary, public key: string, public uuid: string) {
    super(library, key, uuid)
  }

  mapMedia(data: Root<PlexMovie>): PlexMovie[] {
    return data.MediaContainer.Metadata.flatMap((item) => new PlexMovie(this.api, item.ratingKey))
  }
}
...
export class ShowSection extends PlexSection<PlexShow> {
  ...
  mapMedia(data: Root<PlexShow>): PlexShow[] {
    return data.MediaContainer.Metadata.flatMap((item) => new PlexShow(this.api, item.ratingKey))
  }
}
