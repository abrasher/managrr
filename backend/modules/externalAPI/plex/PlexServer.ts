...
export class PlexServer {
  ...
  constructor(public url: string, public token: string) {
    this.client = getPlex(url, token)
    this.library = new PlexLibrary(this)
  }

  async getDetails(): Promise<IPlexServer> {
    const res = await this.client.get<Root>('/').catch((error: AxiosError) => {
      console.log(error)
  ...
  uploadPoster(ratingKey: string | number, poster: Buffer): void {
    this.client.post(`/library/metadata/${ratingKey}/posters`, poster).catch(() => {
