...
import { PosterGenerator } from '@/backend/lib/PosterGenerator'
import { System, Task } from '@/backend/modules/system/system.entity'
import { version } from '@/backend/package.json'
import { getTasks, startTask } from '@/backend/tasks/BaseTask'
...
export class SystemResolver {
  ...
  runTask(@Ctx() context: ContextType, @Arg('input') input: RunTaskInput): string {
    console.log(`Running Task ${input.taskName}`)
    startTask(input.taskName)

    return `${input.taskName} Started`
  }
